...
// MasterChef is the master of Afitx. He can make Afitx and he is a fair guy.
//
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    function safeAfitxTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > afitxBal) {
            transferSuccess = afitx.transfer(_to, afitxBal);
        } else {
            transferSuccess = afitx.transfer(_to, _amount);
        }
        require(transferSuccess, "safeAfitxTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _afitxPerBlock) public onlyOwner {
        massUpdatePools();
        afitxPerBlock = _afitxPerBlock;
        emit UpdateEmissionRate(msg.sender, _afitxPerBlock);
    }
}
